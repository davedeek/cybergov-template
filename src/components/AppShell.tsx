...
const wsNavItems = [
  ...
  { to: '/ws/work-counts', icon: BarChart3, label: 'Work Counts' },
] as const
    ...
    if (path === '/ws') {
      return (
        ...
        (currentPath.startsWith('/ws/') &&
          ...
          !currentPath.startsWith('/ws/work-counts'))
      )
    }
    ...
    <SidebarProvider>
      <Sidebar variant="inset" className="border-r-0">
        ...
        <SidebarContent className="bg-nd-ink">
          <SidebarGroup>
            <SidebarGroupLabel className="text-white/40 uppercase text-[10px] font-mono tracking-[0.2em] mb-2 px-4">
              Application
            </SidebarGroupLabel>
            <SidebarGroupContent>
              <SidebarMenu>
                {navItems.map((item) => (
                  <SidebarMenuItem key={item.to}>
                    ...
                    >
                      ...
                      >
                        <item.icon
                          className={`w-4 h-4 ${isActive(item.to) ? 'text-nd-accent' : 'text-white/50'}`}
                        />
          ...
          <SidebarGroup>
            <SidebarGroupLabel className="text-nd-accent uppercase text-[10px] font-mono tracking-[0.2em] mb-2 px-4">
              Work Simplification
            </SidebarGroupLabel>
            <SidebarGroupContent>
              <SidebarMenu>
          ...
          <SidebarMenu>
            ...
            <SidebarMenuItem>
              <button
                ...
                className="w-full flex items-center gap-3 px-5 py-4 text-[10px] font-mono tracking-[0.2em] uppercase text-white/50 hover:text-nd-accent hover:bg-nd-surface-dark transition-colors text-left border-t border-nd-surface-dark"
              >
      ...
      <div className="flex-1 flex flex-col min-w-0 h-[100dvh] bg-nd-bg overflow-hidden font-sans text-nd-ink w-full relative">
        ...
        <main className="flex-1 overflow-y-auto w-full relative z-0">{children}</main>
      </div>
    </SidebarProvider>
  )
}

            ...
            <div key={step.id}>
              ...
              >
                ...
                <div className="w-12 flex flex-col items-center shrink-0">
                  {idx > 0 && (
                    <div className="w-0.5 flex-1 bg-nd-border group-hover/item:bg-nd-accent/50 transition-colors" />
                  )}
                ...
                >
                  <CardContent className="p-4 flex gap-4 items-start">
                    ...
                    <div className="flex-1">
                      ...
                      <h3
                        className={`text-[15px] leading-snug ${step.symbol === 'operation' ? 'font-bold' : 'font-normal'}`}
                      >
                        {step.description}
                      </h3>
                      ...
                      <div className="flex gap-4 mt-3">
                        {step.minutes && (
                          <div className="flex items-center font-mono text-[10px] text-nd-ink-muted bg-nd-bg px-2 py-0.5 border border-nd-border">
                            ...
                            {(step.minutes ?? 0) > storageWarn && (
                              <span className="text-nd-accent ml-1.5">⚑</span>
                            )}
                          </div>
                        )}
                        {step.feet && (
                      ...
                      {step.notes && (
                        <div className="mt-2 pl-2 border-l-2 border-nd-border text-xs font-mono text-nd-ink-muted italic leading-snug">
                          {step.notes}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col items-end gap-2 shrink-0 opacity-0 group-hover/item:opacity-100 transition-opacity">
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-7 w-7 text-nd-border hover:text-nd-accent"

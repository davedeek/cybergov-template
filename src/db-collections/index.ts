...
const collectionsCache = new Map<string, ReturnType<typeof createCollection>>()

function getCachedCollection<T extends ReturnType<typeof createCollection>>(key: string, factory: () => T): T {
    ...
    return getCachedCollection(`todos-${orgId}`, () =>
      createCollection(
        queryCollectionOptions({
          ...
          getKey: (todo) => (todo as { id: number }).id,
    ...
    return getCachedCollection(`members-${orgId}`, () =>
      createCollection(
        queryCollectionOptions({
          ...
          getKey: (member) => (member as { id: number }).id,
        })
      )
    )
  }, [orgId])
    ...
    return getCachedCollection(`units-${orgId}`, () =>
      createCollection(
        queryCollectionOptions({
          ...
          getKey: (unit) => (unit as { id: number }).id,
          onInsert: async ({ transaction }) => {
            if (!orgId) return
            const results = []
            for (const m of transaction.mutations) {
    ...
    return getCachedCollection(`pc-${orgId}-${unitId}`, () =>
      createCollection(
        queryCollectionOptions({
          ...
          getKey: (chart) => (chart as { id: number }).id,
          onInsert: async ({ transaction }) => {
    ...
    return getCachedCollection(`steps-${orgId}-${pcId}`, () =>
      createCollection(
        queryCollectionOptions({
          ...
          getKey: (step) => (step as { id: number }).id,
          onInsert: async ({ transaction }) => {
    ...
    return getCachedCollection(`wdc-employees-${orgId}-${wdcId}`, () =>
      createCollection(
        queryCollectionOptions({
          ...
          getKey: (emp) => (emp as { id: number }).id,
        })
      )
    )
  }, [orgId, wdcId])
    ...
    return getCachedCollection(`wdc-activities-${orgId}-${wdcId}`, () =>
      createCollection(
        queryCollectionOptions({
          ...
          getKey: (act) => (act as { id: number }).id,
        })
      )
    )
  }, [orgId, wdcId])
    ...
    return getCachedCollection(`wdc-tasks-${orgId}-${wdcId}`, () =>
      createCollection(
        queryCollectionOptions({
          ...
          getKey: (task) => (task as { id: number }).id,
        })
      )
    )
  }, [orgId, wdcId])

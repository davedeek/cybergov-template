...
import { useAllProcessChartsCollection } from '@/db-collections'
import { GitBranch, Calendar, ArrowRight, Home } from 'lucide-react'

import { Card, CardContent, CardHeader, CardTitle, CardFooter } from '@/components/ui/card'
import { Button } from '@/components/ui/button'

export const Route = createFileRoute('/_authed/ws/process-charts')({
  component: AllProcessChartsPage,
  head: () => ({
    meta: [{ title: 'Process Charts — CyberGov' }],
  }),
})

function AllProcessChartsPage() {
  const trpc = useTRPC()
  ...
  const pcCollection = useAllProcessChartsCollection(orgId)
  const { data: pcList = [], isLoading } = useLiveQuery(
  ...
  return (
    <div className="p-6 lg:p-10 max-w-7xl mx-auto bg-nd-bg min-h-full">
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-heading font-bold text-nd-ink">All Process Charts</h1>
        <p className="text-nd-ink-muted mt-2">
          View and manage all step-by-step process documentation across your organization.
        </p>
      </div>
        ...
        <Card className="border-nd-border bg-nd-surface shadow-sm text-center py-16">
          <CardContent>
            <div className="w-16 h-16 bg-nd-accent-light rounded-full flex items-center justify-center mx-auto mb-4">
              <GitBranch className="w-8 h-8 text-nd-accent" />
            </div>
            <h3 className="text-xl font-heading font-semibold text-nd-ink mb-2">No Process Charts Found</h3>
            <p className="text-nd-ink-muted mb-6 max-w-sm mx-auto">
              You haven't created any process charts yet. Go to a unit dashboard to create your first chart.
            </p>
              ...
              <CardFooter className="pt-0">
                <Button asChild variant="outline" className="w-full justify-between hover:bg-nd-bg hover:text-nd-accent">
                  <Link
                    ...
                    params={{ unitId: chart.unitId.toString(), pcId: chart.id.toString() }}
                    search={orgId ? { orgId } : {}}
                  >
                    Open Chart
